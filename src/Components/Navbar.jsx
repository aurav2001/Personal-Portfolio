        ...
        <div className={`
              ...
              {navItems.map((item, index) => {
                ...
                return (
                  ...
                  >
                    {isHomePage ? (
                      <AnchorLink
                        className={`group flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 ${isActive ? 'bg-gradient-to-r from-primary/20 to-accent/20 border border-primary/30 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5 border border-transparent'}`}
                        href={item.toLowerCase() === "home" ? "#home" : `#${slug}`}
                        offset={100}
                        onClick={() => { setMenu(slug); closeMenu(); }}
                      >
                        <div className={`${isActive ? 'text-primary' : 'text-gray-500 group-hover:text-primary'} transition-colors duration-300`}>{icons[slug]}</div>
                        <span className="text-xl font-bold">{item}</span>
                      </AnchorLink>
                    ) : (
                      <Link
                        to={item.toLowerCase() === "home" ? "/#home" : `/#${slug}`}
                        className={`group flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 ${isActive ? 'bg-gradient-to-r from-primary/20 to-accent/20 border border-primary/30 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5 border border-transparent'}`}
                        onClick={() => { setMenu(slug); closeMenu(); }}
                      >
                        <div className={`${isActive ? 'text-primary' : 'text-gray-500 group-hover:text-primary'} transition-colors duration-300`}>{icons[slug]}</div>
          ...
          <div className="p-6 border-t border-white/5">
            {isHomePage ? (
              ...
              <Link
                to="/#contact"
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-primary to-accent text-white font-bold text-center flex items-center justify-center gap-2 transition-all duration-300"
                onClick={closeMenu}
              >
                <span>Let's Connect</span>
              </Link>
